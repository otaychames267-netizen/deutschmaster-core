...
export const startPdfExtraction = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const prevCompleted = Array.isArray(prevMeta.chunks_completed)
      ...
      : [];
    // Auto-resume when there are already completed chunks and the caller did
    // not explicitly pass resume:false. This prevents "Extrahieren" from
    // silently discarding a partially-complete extraction; only an explicit
    // force-reset (resume === false) starts from scratch.
    const resume = data.resume !== false && prevCompleted.length > 0;
...
export const extractPdfChunk = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      // Renew the watchdog lease before the Gemini call so a retrying chunk
      // (up to 4 attempts × 85 s + backoffs ≈ 364 s) cannot be killed by the
      // 15-minute watchdog between two successful-chunk updates.
      await context.supabase
        .from("pdf_imports")
 ...
 * 15 minutes as failed. Called from the admin page poller so orphaned rows
...
export const reapStuckExtractions = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    await assertAdmin(context);
    // 15 min: a single chunk can retry up to 4 times with backoffs totalling
    // ~364 s. 5 min was too short and caused the watchdog to kill live chunks.
    const cutoffISO = new Date(Date.now() - 15 * 60 * 1000).toISOString();
    // Extracting jobs older than 15 min
      ...
      await context.supabase
        ...
        .update({
          ...
          error_message:
            ...
            row.status +
            "' for more than 15 minutes — auto-failed. Use 'Resume' to continue from the last completed chunk.",
        })
        .eq("id", row.id);
      reaped++;
    }
    // Building jobs older than 15 min
    ...
    for (const row of stuckBuild ?? []) {
      await context.supabase
        ...
        .update({
          status: "build_failed",
          error_message: "[watchdog] Build stuck for more than 15 minutes — auto-failed.",
        })
        .eq("id", row.id);
      reaped++;
    }
...
/**
 * Publish a draft exercise (super_admin only).
 */
...
/**
 * Run a 100% fidelity check between the original PDF extraction and built exercises.
 */
