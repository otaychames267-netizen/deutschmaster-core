 ...
 * tiers. Pricing is NOT overridden at checkout time — the LS variant's own
 * configured price is authoritative and must be kept in sync with the
 * `plans` table's price_tnd manually whenever either changes (an earlier
 * version tried to force-override via checkout_data.custom_price, but Lemon
 * Squeezy only honors that field when the variant has "pay what you want"
 * pricing enabled; on a fixed-price variant it 422s with a misleading
 * "checkout data field must be an array" error — confirmed live against the
 * real API on 2026-07-22, see the Lemon Squeezy activation checklist).
 *
