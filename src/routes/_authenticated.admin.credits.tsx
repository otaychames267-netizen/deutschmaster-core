...
function AdminCreditsPage() {
  ...
  const loadStats = useCallback(async () => {
    ...
    const [{ data: txData }, { data: activeData }] = await Promise.all([
      supabase
        ...
        .limit(25),
      supabase.from("student_credits").select("user_id").gt("balance", 0),
    ]);

    let granted = 0, spent = 0;
    const { data: allTx } = await supabase.from("credit_transactions").select("delta");
    for (const t of allTx ?? []) {
      if (t.delta > 0) granted += t.delta;
  ...
  async function handleSearch() {
    ...
    const { data: balances } = await supabase.from("student_credits").select("user_id, balance").in("user_id", ids);
    const balanceMap = new Map((balances ?? []).map((b) => [b.user_id, b.balance]));
    setHits(profiles.map((p) => ({ id: p.id, full_name: p.full_name, email: p.email, balance: balanceMap.get(p.id) ?? 0 })));
    setSearching(false);
  }
  ...
  async function handleGrant() {
    ...
    toast.success(`Granted ${amt} credits to ${selected.email ?? selected.full_name}. New balance: ${newBalance}.`);
    setSelected({ ...selected, balance: newBalance as number });
    setNote("");
    loadStats();
  }
    ...
    <div className="mx-auto max-w-4xl space-y-6 pb-8">
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-xl font-black tracking-tight text-foreground">Essay Grading Credits</h1>
        </div>
        <p className="text-sm text-muted-foreground ml-12">Grant students credits for AI essay grading. No self-serve top-up yet — manual only.</p>
      </div>

      {/* Stats */}
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { label: "Total granted", value: stats.granted, icon: TrendingUp, color: "text-emerald-500 bg-emerald-500/10" },
          { label: "Total spent", value: stats.spent, icon: TrendingDown, color: "text-blue-500 bg-blue-500/10" },
      ...
      <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-6">
        <p className="font-bold text-foreground mb-4">Grant credits to a student</p>
          ...
          <div className="mb-4 space-y-1.5">
            {hits.map((h) => (
              <button
                ...
                className="flex w-full items-center justify-between rounded-xl border border-border bg-card px-4 py-2.5 text-left text-sm hover:border-emerald-500/40 transition-colors"
              >
                <span>
                  <span className="font-medium text-foreground">{h.full_name ?? "—"}</span>
                  <span className="text-muted-foreground"> · {h.email}</span>
                </span>
                <span className="text-xs font-semibold text-emerald-600">{h.balance} credits</span>
              </button>
            ))}
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3">
              ...
              <div className="text-right">
                ...
                <p className="text-sm font-bold text-emerald-600">{selected.balance} credits</p>
              </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-foreground">Credits to grant</label>
                <input
                  type="number"
                  min={1}
                  value={amount}
            ...
            <div className="flex justify-end">
              <button
                onClick={handleGrant}
                disabled={granting}
                className="flex items-center gap-2 rounded-xl bg-emerald-500 px-4 py-2.5 text-sm font-bold text-white hover:bg-emerald-600 transition-colors disabled:opacity-60"
              >
                <Plus className="h-4 w-4" /> Grant credits
              </button>
            </div>
          </div>
        )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {loadingTx ? (
                ...
                <tr><td colSpan={4} className="px-6 py-6 text-center text-muted-foreground">No credit activity yet.</td></tr>
              ) : (
                transactions.map((t) => (
                  <tr key={t.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    <td className={`px-6 py-3.5 font-semibold ${t.delta > 0 ? "text-emerald-600" : "text-destructive"}`}>
