...
export const Route = createFileRoute("/_authenticated/$level/dashboard")({
...
function DashboardPage() {
  ...
  // other level's in-progress exercise here (strict isolation).
  const [lastLesson] = useState<LastLesson | null>(() => {
    const l = getLastLesson();
    return l && l.path.startsWith(`/${seg}/`) ? l : null;
  });
    ...
    (async () => {
      ...
      const [profileRes, subRes, attemptsRes, goalRes, refRes] = await Promise.all([
        supabase.from("profiles")
          .select("full_name, onboarding_completed, exam_date, referral_code")
        ...
        supabase.from("attempt_results")
          .select("score, created_at, exams!inner(level)")
          .eq("user_id", user.id)
          .eq("exams.level", activeLevel)
        ...
        <div className={`grid gap-5 ${muendlichVisible ? "md:grid-cols-2" : "md:grid-cols-1"}`}>
          ...
          >
            ...
            <div className="relative">
              ...
              <p className="mt-0.5 text-sm text-blue-100/70">Written exam · TELC {levelBadge}</p>
              ...
              <div className="mt-5 grid grid-cols-2 gap-3">
                ...
                <Link
                  to={`/${seg}/schriftlich/pruefung` as never}
          ...
          >
            ...
            <div className="relative">
              ...
              <p className="mt-0.5 text-sm text-rose-100/70">Oral exam · TELC {levelBadge}</p>
        ...
        <div className="col-span-full rounded-2xl border border-border bg-card p-5 shadow-sm lg:col-span-3">
          ...
          {weeklyData.some(d => d.score > 0) ? (
            ...
            <div className="flex h-44 flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-border">
              ...
              <Link to={`/${seg}/schriftlich` as never} className="rounded-xl bg-primary px-4 py-1.5 text-xs font-bold text-primary-foreground hover:bg-primary/90 transition-colors">
                Start practising
              </Link>
            </div>
          )}
