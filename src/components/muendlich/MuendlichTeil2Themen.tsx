/**
 * Mündlich Teil 2 main view — Hero Card grid (owner decision 2026-08-10:
 ...
 * real row opens the full native content modal (all 7 pages: Text, Inhalt,
 * Meinung, Erfahrung, Vor-/Nachteile, Prüfungsfragen, Wortschatz —
 * everything that used to be PDF-only); an empty result opens the paywall
 * instead. The grid itself (which topics exist, including ones never
 * introduced in a Tunisian exam center) is always visible via the
 * titles-only catalog RPC — browsing costs nothing, only opening a topic
...
import { Teil2TopicModal, type Teil2TopicRow } from "./MuendlichTeil2TopicModal";

export function MuendlichTeil2Themen() {
  const level = useActiveLevel();
  const catalog = useMuendlichCatalog(2, level);
  const [openTopic, setOpenTopic] = useState<Teil2TopicRow | null>(null);
  ...
  async function openTopicModal(id: string) {
    ...
    const { data } = await supabase
      ...
      .maybeSingle();
    setFetchingId(null);
    if (data) setOpenTopic(data as Teil2TopicRow);
  ...
  return (
    <div className="space-y-8">
      ...
      {openTopic && <Teil2TopicModal topic={openTopic} onClose={() => setOpenTopic(null)} />}
