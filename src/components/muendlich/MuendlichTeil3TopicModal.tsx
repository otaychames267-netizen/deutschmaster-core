...
const REDEMITTEL_LIBRARY: Record<string, { emoji: string; label: string; frage: string[]; antwort: string[] }> = {
  ...
  berufe: { emoji: "👷", label: "Berufe", frage: ["Welche Berufe sollten wir vorstellen?", "Was für Berufe könnten die Schüler besonders interessieren?", "Sollten wir eher bekannte oder ungewöhnliche Berufe zeigen?"], antwort: ["Ich würde vorschlagen, möglichst unterschiedliche Berufe zu zeigen.", "Das halte ich für sinnvoll, so ist für jeden etwas dabei.", "Das wäre sicher eine interessante Auswahl."] },
  praesentationsform: { emoji: "🎬", label: "Präsentationsform", frage: ["Wie sollten wir den Inhalt präsentieren?", "Was hältst du von einem Video statt eines reinen Vortrags?", "Wie machen wir es besonders anschaulich?"], antwort: ["Am anschaulichsten wäre es wahrscheinlich, ein Video zu zeigen.", "Das finde ich eine gute Idee, das wirkt lebendiger.", "Das würde die Zuhörer sicher mehr ansprechen."] },
  unterrichtsbeispiele: { emoji: "📚", label: "Unterrichtsbeispiele", frage: ["Welche Unterrichtsbeispiele sollten wir zeigen?", "Was für ein Beispiel würde das Thema gut veranschaulichen?", "Sollten wir ein praktisches Beispiel vorführen?"], antwort: ["Ich würde vorschlagen, ein konkretes Beispiel zu zeigen.", "Das halte ich für eine gute Idee, das ist bestimmt sehr anschaulich.", "Das würde den Teilnehmern sicher weiterhelfen."] },
  einrichtungen: { emoji: "🏛️", label: "Einrichtungen", frage: ["Welche Einrichtungen sollten wir besuchen?", "Was würde die Gruppe am meisten interessieren?", "Sollten wir mehrere verschiedene Orte einplanen?"], antwort: ["Ich hätte da eine Idee: eine Schule, eine Universität und eine Bibliothek.", "Das finde ich gut, so bekommen sie einen guten Überblick.", "Das wäre sicher lehrreich für die Gruppe."] },
  textsorten: { emoji: "📝", label: "Textsorten", frage: ["Welche Textsorten sollten wir zulassen?", "Sollten wir eher freie oder feste Themen vorgeben?", "Was für Texte würden den Teilnehmern liegen?"], antwort: ["Ich würde vorschlagen, mehrere Textsorten zuzulassen.", "Das halte ich für sinnvoll, so kann jeder seine Stärken zeigen.", "Das wäre sicher eine faire Lösung."] },
  preise: { emoji: "🏆", label: "Preise", frage: ["Was für Preise sollten wir vergeben?", "Wie viele Gewinner sollte es geben?", "Sollten wir auch einen Trostpreis einplanen?"], antwort: ["Am sinnvollsten wäre es wahrscheinlich, die besten drei zu prämieren.", "Das klingt nach einer schönen Idee.", "Das würde die Teilnehmer sicher zusätzlich motivieren."] },
  herausforderungen: { emoji: "🧩", label: "Herausforderungen", frage: ["Welche Herausforderungen könnte es dabei geben?", "Woran sollten wir besonders denken?", "Was könnte am schwierigsten sein?"], antwort: ["Ich denke, das könnte vor allem an … liegen.", "Das halte ich für einen wichtigen Punkt.", "Wir sollten das von Anfang an mitbedenken."] },
  etappen: { emoji: "🚴", label: "Etappen", frage: ["Wie weit sollten wir täglich fahren?", "Wie sollten wir die Strecke aufteilen?", "Was ist eine realistische Tagesetappe für die Gruppe?"], antwort: ["Am sinnvollsten wäre es wahrscheinlich, nicht zu viele Kilometer pro Tag einzuplanen.", "Das klingt nach einer vernünftigen Lösung.", "So wird es für alle machbar."] },
  gepaeck: { emoji: "🎒", label: "Reisegepäck", frage: ["Wie sollte das optimale Reisegepäck aussehen?", "Was sollten wir auf keinen Fall vergessen?", "Sollten wir möglichst leicht packen?"], antwort: ["Wir sollten möglichst leicht packen, nur das Nötigste.", "Das halte ich für sehr wichtig.", "Sonst wird es unterwegs zu anstrengend."] },
};
