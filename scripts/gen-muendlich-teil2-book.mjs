...
async function main() {
  ...
  async function renderVariant(groups, label, topicCount) {
    ...
    writeFileSync(`${SCRATCH}\\muendlich-teil2-meisterbuch-${label}.html`, html, "utf8");
    const page = await browser.newPage();
    await page.setContent(html, { waitUntil: "networkidle" });
    await page.evaluate(() => document.fonts.ready);
    await page.waitForTimeout(500);
    const pdfPath = `${SCRATCH}\\muendlich-teil2-meisterbuch-${label}.pdf`;
  ...
  const studentStoragePath = "teil-2/redemittel/AuraLingovia-Sprechen-Teil2-Meisterbuch.pdf";
  const adminStoragePath = "teil-2/redemittel/admin/AuraLingovia-Sprechen-Teil2-Meisterbuch-Admin.pdf";
  ...
  const title = `Sprechen Teil 2 — Das Meisterbuch (${activeCount} Themen)`;
  if (existing) {
    ...
    const { data: ins, error: insErr } = await db.from("muendlich_materials").insert({
      teil: 2, category: "redemittel", level, title,
